    ...
    def __init__(self,
        ...
        ti.root.dense(ti.ij,
                      render_resolution).dense(ti.ij,
                                               (8, 8)).place(self.rays, self.rays.grad)
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.batched: # Batched Gradient
            ...
            for i, vol, tf, lf in zip(range(ctx.bs), vols, tfs, lfs):
                ...
                # print('Output RGBA', torch.nan_to_num(ctx.vr.output_rgba.grad.to_torch(device='cpu')).abs().max())
                # print('Render Tape', torch.nan_to_num(ctx.vr.render_tape.grad.to_torch(device='cpu')).abs().max())
                ...
                # print('Entry', torch.nan_to_num(ctx.vr.entry.grad.to_torch(device='cpu')).abs().max())
                # print('Exit', torch.nan_to_num(ctx.vr.entry.grad.to_torch(device='cpu')).abs().max())
                # print('Rays', torch.nan_to_num(ctx.vr.rays.grad.to_torch(device='cpu')).abs().max())
...
class Raycaster(torch.nn.Module):
    def __init__(self, volume_shape, output_shape, tf_shape, sampling_rate=1.0, jitter=True, background_color=0.0, max_samples=512, fov=30.0, near=0.1, far=100.0, ti_kwargs={}):
        super().__init__()

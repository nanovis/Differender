    ...
    def raycast(self, cam_pos_x: float, cam_pos_y: float, cam_pos_z: float, sampling_rate: float):
        ...
        for i, j in self.render: # For all pixels
            ...
            if hit: # Shoot the ray
                ...
                t = tmin + 0.5 * t_inc # Start of ray
                color = tl.vec4(0.0) # Color to be composited
                cnt = 0
                while cnt < n_samples and color.w < 0.99:
                    pos = look_from + t * vd
                    intensity = self.sample_volume_trilinear(pos)
                    s_color = self.apply_transfer_function(intensity) # Sample Color
                    s_color = tl.vec4(s_color.xyz * s_color.w, s_color.w)
                    color += (1.0 - color.w) * s_color
                    t += t_inc
                    cnt += 1
...
if __name__ == '__main__':
    ...
    tfgen = TFGenerator()
    tf = tex_from_pts(tfgen.generate(), 128).numpy()
